...
function App() {
  ...
  const displayProjectsSoftware = softwareProjectsData.map((data, key) => {
    return (
      <ProjectsSoftware key={key} name={data.name} description={data.description} appLink={data.appLink}
        image={data.image} repoFrontLink={data.repoFrontLink} repoBackLink={data.repoBackLink} />
    );
  });
  ...
  const displayProjectsMaker = makerProjectsData.map((data, key) => {
    console.log(data)
    return (
      <ProjectsMaker key={key} name={data.name} description={data.description} link={data.link} image={data.image} />
    );
  });
  ...
  return (
    <div className="App">
      ...
      <div className="portfolio-projects card-group">
        ...
        <hr className="rule" />
        <p>
          This engineer is more than meets the eye. There are a number of paths
          that lead into the world of software development. Here are just a sample
          of the things that got me to where I am today:
      </p>
        <div className="portfolio-projects card-group container">{displayProjectsMaker}</div>
      </div>
    </div>
  );
}
